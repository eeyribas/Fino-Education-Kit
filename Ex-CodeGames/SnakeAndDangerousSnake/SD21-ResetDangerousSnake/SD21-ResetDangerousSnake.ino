// Snake And Dangerous Snake - Reset The Dangerous Snake
...
const int UP = 3;
int direction = RIGHT;
int direction_2 = DOWN;

// Dangerous snake's coordinate arrays are defined.
int dangerous_snake_x[64];                   
int dangerous_snake_y[64];  

// The variables related to the dangerous snake are identified.
int dangerous_snake_length = 1;
...
// Game over check.
    ...
    for (int i = snake_length - 1; i > 0; i--) {
      for (int j = dangerous_snake_length - 1; j > 0; j--) {
        if (snake_x[i] == dangerous_snake_x[j] && snake_y[i] == dangerous_snake_y[j])
          ResetGame();
      }
    }

// Determination of direction information for the Dangerous snake occurs.
    int tmp_dir = random(0, 4);
    if (tmp_dir == 0) {
      direction_2 = DOWN; 
    } else if (tmp_dir == 1) {
      direction_2 = UP;
...
// The array values of the dangerous snake are entered.
    for (int i = dangerous_snake_length - 1; i > 0; i--) {
      dangerous_snake_x[i] = dangerous_snake_x[i - 1];
      dangerous_snake_y[i] = dangerous_snake_y[i - 1];
    }

    switch (direction_2) {
      case RIGHT:
        if (dangerous_snake_y[0] > 0) {
          dangerous_snake_y[0] = dangerous_snake_y[0] - 1;
        } else {
          dangerous_snake_y[0] = 7;
        }
        break;

      case DOWN:
        if (dangerous_snake_x[0] < 7) {
          dangerous_snake_x[0] = dangerous_snake_x[0] + 1;
        } else {
          dangerous_snake_x[0] = 0;
        }
        break;

      case LEFT:
        if (dangerous_snake_y[0] < 7) {
          dangerous_snake_y[0] = dangerous_snake_y[0] + 1;
        } else {
          dangerous_snake_y[0] = 0;
        }
        break;

      case UP:
        if (dangerous_snake_x[0] > 0) {
          dangerous_snake_x[0] = dangerous_snake_x[0] - 1;
        } else {
          dangerous_snake_x[0] = 7;
        }
        break;
    }
...
// The dangerous snake's apple-catching condition is checked. 
// If the dangerous snake catches the apple, it grows.
    if ((dangerous_snake_x[0] == apple_x) && (dangerous_snake_y[0] == apple_y)) {
      dangerous_snake_length++;

      if (dangerous_snake_length < 64) {      
        apple_x = random(0, 8);
        apple_y = random(0, 8);
...
// The dangerous snake's drawing is carried out.
  for (int i = 0; i < dangerous_snake_length; i++) {     
    fino.SetDot(dangerous_snake_x[i], dangerous_snake_y[i], HIGH);         
  }
...
boolean CatchingApple(int tmp_apple_x, int tmp_apple_y)
{
  ...
  for (int i = 0; i < dangerous_snake_length - 1; i++) {
    if ((tmp_apple_x == dangerous_snake_x[i]) && (tmp_apple_y == dangerous_snake_y[i])) {
      return true;
    }
  }
...
// Assigning values to arrays for Snake drawings.
  ...
  dangerous_snake_length = 1;
  
// Assigning values to arrays for Dangerous snake drawings.
  dangerous_snake_x[0] = 4;
  dangerous_snake_y[0] = 1;

  for (int i = 1; i < 64; i++)
    dangerous_snake_x[i] = dangerous_snake_y[i] = -1;
...
// In the reset state, LEDs will turn on and off.
  fino.RedLedHigh();
  fino.YellowLedHigh();
  delay(2000);
  fino.RedLedLow();
